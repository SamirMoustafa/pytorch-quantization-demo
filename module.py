...
def calcScaleZeroPoint(min_val, max_val, num_bits=8):
    qmin = Parameter(tensor(0.0, requires_grad=True))
    qmax = Parameter(tensor(2.0 ** num_bits - 1.0, requires_grad=True))
...
class QParam(nn.Module):
    def __init__(self, num_bits=8):
        ...
        self.scale = torch.tensor([], requires_grad=True)
        self.zero_point = torch.tensor([], requires_grad=True)
...
class QModule(nn.Module):
    def __init__(self, qi=True, qo=True, num_bits=8):
        ...
        if qi:
            ...
            self.is_input_initialized = False
        if qo:
...
class QLinear(QModule):
    ...
    def quantize_inference(self, x):
        ...
        x = self.fc_module(x)
        x = (self.qw.scale * self.qi.scale / self.qo.scale) * x.round()
...
class QConvBNReLU(QModule):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            self.bn_module.running_mean = (
                    self.bn_module.momentum * self.bn_module.running_mean + (1 - self.bn_module.momentum) * mean
            )
            self.bn_module.running_var = (
                    self.bn_module.momentum * self.bn_module.running_var + (1 - self.bn_module.momentum) * var
            )
        else:
    ...
    def freeze(self, qi=None, qo=None):
        ...
        self.conv_module.bias.data = quantize_tensor(
            bias, scale=self.qi.scale * self.qw.scale, zero_point=0, num_bits=32, signed=True
        )
